...
- Fixed spawn limits counting spawner spawns twice on Forge and NeoForge, blocking extra spawns.
...
- Fixed merged item drops staying in the world as empty entities, raising the entity count.
- Fixed spawn statistics counting excluded spawns while the `spawn` feature was disabled.
...
- Fixed the login and teleport warmup never triggering for teleports inside the same dimension, like
  commands, ender pearls and teleports from other mods.
- Fixed a possible crash while a player joined the server with an active chunk generation throttle.
- Fixed `movementThrottleSampleTicks` and `movementThrottleWindowSamples` falling back to their
  defaults on every server start until the first config reload.
- Fixed view distance not recovering from the login warmup while a player kept moving with
  `movementWarmupEnabled=false`.
- Fixed the movement speed being overestimated when position samples were delayed by server lag.
